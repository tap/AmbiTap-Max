...
public:
    ...
    attribute<bool> max_re {
        this, "max_re", false,
        description {"Apply per-order max-rE weighting to reduce side lobes."},
        setter {MIN_FUNCTION {
    ...
    void operator()(audio_bundle input, audio_bundle output) {
        ...
        for (auto i = 0; i < frames; ++i) {
            ...
            m_decoder->process_frame(m_in_frame.data(), m_out_frame.data(),
                                     static_cast<size_t>(m_speaker_count));
...
private:
    static std::vector<ambitap::spherical_coord> layout_from_name(const std::string& name) {
        ...
        if (name == "surround_5_1" || name == "5.1") return surround_5_1();
        if (name == "surround_7_1" || name == "7.1") return surround_7_1();
        if (name == "surround_7_1_4" || name == "7.1.4") return surround_7_1_4();
        if (name == "cube") return cube();
        if (name == "hexagon") return hexagon();
        if (name == "octagon") return octagon();
        return {};
    }

    static ambitap::dsp::decoder_algorithm algorithm_from_name(const std::string& name) {
        if (name == "allrad") return ambitap::dsp::decoder_algorithm::allrad;
        if (name == "epad") return ambitap::dsp::decoder_algorithm::epad;
