...
class ambitap_panbin : public object<ambitap_panbin>, public vector_operator<> {
    ...
    struct convolver_pair {
        ...
        convolver_pair(size_t block_size, const std::vector<float>& left_ir,
                       const std::vector<float>& right_ir)
...
private:
    ...
    std::unique_ptr<convolver_pair> build_pair() {
        ...
        ambitap::evaluate_sh(ambitap::builtin_hrtf_order,
                             static_cast<float>(m_azimuth_value),
        ...
        if (host_rate != ambitap::builtin_hrtf_sample_rate) {
            left  = ambitap::resample_fir(left.data(), left.size(),
                                          ambitap::builtin_hrtf_sample_rate, host_rate);
            right = ambitap::resample_fir(right.data(), right.size(),
                                          ambitap::builtin_hrtf_sample_rate, host_rate);
        }
