...
public:
    ...
    explicit ambitap_room(const atoms& args = {}) {
        int order = 1;
        if (!args.empty())
            order = std::clamp(static_cast<int>(args[0]), 0,
                               ambitap::dsp::room::k_max_room_order);
    ...
    attribute<number> listener_x {
        this, "listener_x", 1.746,
    ...
    attribute<number> listener_y {
        this, "listener_y", 1.711,
    ...
    attribute<number> listener_z {
        this, "listener_z", 0.668,
    ...
    attribute<bool> er {
        this, "er", true,
        description {"Enable the image-source early reflections (first 30 ms)."},
        setter {MIN_FUNCTION {
            const bool on = args[0];
            if (m_room)
    ...
    attribute<bool> tail {
        this, "tail", true,
        description {"Enable the FDN late-reverb tail (t >= 30 ms)."},
        setter {MIN_FUNCTION {
            const bool on = args[0];
            if (m_room)
    ...
    attribute<number> gain {
        this, "gain", 1.0,
        description {"Linear output gain (smoothed per sample)."},
        setter {MIN_FUNCTION {
            const double v = args[0];
    ...
    message<> rt60band {
        this, "rt60band", "Set one octave band's RT60: rt60band <center_hz> <seconds>.",
        MIN_FUNCTION {
...
private:
    ...
    void push_source(double x, double y, double z) {
        if (m_room)
            m_room->set_source_position(static_cast<float>(x), static_cast<float>(y),
                                        static_cast<float>(z));
    }
    void push_listener(double x, double y, double z) {
        if (m_room)
            m_room->set_listener_position(static_cast<float>(x), static_cast<float>(y),
                                          static_cast<float>(z));
    }
