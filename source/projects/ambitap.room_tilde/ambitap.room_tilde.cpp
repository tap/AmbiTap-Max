...
attribute<number> dim_y{this, "dim_y", 5.30, description{"Room width in meters (y, left axis). Clamped to 1..50 m."},
                        setter{MIN_FUNCTION{const double v = args[0];
...
attribute<symbol> absorption{this, "absorption", "fir",
                             ...
                             setter{MIN_FUNCTION{using kind = ambitap::dsp::room::absorption_kind;
...
message<> rt60band{this, "rt60band", "Set one octave band's RT60: rt60band <center_hz> <seconds>.",
                   MIN_FUNCTION{if (args.size() < 2 || !m_room){return {};
}
...
message<> reflections{this, "reflections", "Set the six wall reflection coefficients: reflections <x0 x1 y0 y1 z0 z1>.",
                      MIN_FUNCTION{if (args.size() < 6 || !m_room){return {};
}
