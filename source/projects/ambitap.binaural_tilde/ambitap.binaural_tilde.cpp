...
class ambitap_binaural : public object<ambitap_binaural>, public mc_operator<> {
  ...
  public:
    ...
    attribute<number> volume{this, "volume", 1.0, description{"Linear output gain (post-convolution)."},
...
attribute<symbol> hrtf_dataset{this, "hrtf_dataset", "ls",
                               ...
                               setter{MIN_FUNCTION{if (m_renderer){const std::string name = args[0];
...
attribute<symbol> sofa{this, "sofa", "",
...
try {
    auto loaded = std::make_unique<ambitap::hrtf_data>(ambitap::load_sofa(resolve_in_search_path(requested)));
...
#else
                           if (!static_cast<std::string>(args[0]).empty()) cerr
                           << "this build has no SOFA support (AMBITAP_ENABLE_SOFA was off)" << endl;
return args;
#endif
}
}
...
attribute<number> yaw{this, "yaw", 0.0, description{"Head-tracking yaw about +Z (up), radians. Applied first."},
                      setter{MIN_FUNCTION{const double v = args[0];
if (m_renderer) {
...
attribute<number> pitch{this, "pitch", 0.0,
...
attribute<number> roll{this, "roll", 0.0, description{"Head-tracking roll about +X (front), radians. Applied last."},
                       setter{MIN_FUNCTION{const double v = args[0];
if (m_renderer) {
...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    m_renderer->process(m_in_ptrs.data(), m_left_buf.data(), m_right_buf.data(), static_cast<size_t>(frames));
