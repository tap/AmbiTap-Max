...
public:
    ...
    attribute<number> threshold {
        this, "threshold", -12.0, description {"Compression threshold in dB."},
        setter {MIN_FUNCTION {
    ...
    attribute<number> ratio {
        this, "ratio", 4.0, description {"Compression ratio. 1 = no compression."},
        setter {MIN_FUNCTION {
            const double v = args[0];
            if (m_compressor)
    ...
    attribute<number> attack {
        this, "attack", 0.005, description {"Attack time in seconds."},
        setter {MIN_FUNCTION {
            const double v = args[0];
            if (m_compressor)
    ...
    attribute<number> release {
        this, "release", 0.1, description {"Release time in seconds."},
        setter {MIN_FUNCTION {
            const double v = args[0];
            if (m_compressor)
    ...
    attribute<number> makeup_gain {
        this, "makeup_gain", 0.0, description {"Makeup gain in dB."},
        setter {MIN_FUNCTION {
            const double v = args[0];
            if (m_compressor)
